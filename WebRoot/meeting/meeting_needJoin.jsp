...
<html>
	<head>
		...
		<script>
			function isJoin(that,meetid){  	//meeting的meetId
				...
				$.ajax({
	               ...
	               data:{meetid:meetid,state:$(that).text()},
	               type:"post",
			...
			$("#dg").datagrid({    
		    		url:"${path}/meeting/selectPageUseDyc.action?retime="+new Date().getTime(),    
		        				...
		        				{field:"state",title:"是否参加",width:150,align:"center",
		        						formatter:function(value,row,text){
											if(value==1){     //预参加状态（默认,让人有选择）
												return "<button type='button' class='btn btn-outline-success "+row.meetid+"' onclick=\"isJoin(this,'"+row.meetid+"')\">是</button>"+"<span id='"+row.meetid+"'>/</span>"+
													   "<button type='button' class='btn btn-outline-success "+row.meetid+"' onclick=\"isJoin(this,'"+row.meetid+"')\">否</button>"
  	...
  	<style>
  	    /* 在common.css定义的display被覆盖，所以在这定义。这样就不会别覆盖啦！！ */
		 .hidden{
				display: none;
		  } 
	</style>
