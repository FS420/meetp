...
<body background="../img/picture3.jpg">
	...
	<script>
		...
		$("#dg").datagrid({
			url : "${path}/meeting/selectPageUseDyc.action?flag=already&retime="
					+ new Date().getTime(),
			...
			columns : [ [
					...
					},
					{
						...
						formatter : function(value, row, text) {
							if (value == 1) { //预参加状态（默认,让人有选择）
								return "<input type='button' class='"
										+ row.meetid
										+ "' onclick=\"isJoin(this,'"
										+ row.meetid
										+ "')\" value='是'/>"
										+ "<span id='"+row.meetid+"'>/</span>"
					...
					}, 
					{
						...
						align : "center",
						formatter : function(value, row, text) {
							return "<button onclick=\"uploadUI('"+value+"')\">上传文件</button>";	
 						}
					},
				] ],
		});
